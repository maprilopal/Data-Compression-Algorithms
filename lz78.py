    ...
    try:
        ...
        if tex != "":
            ...
            for i in range(len(alph)):
                diction[alph[i]] = fromDecToBin(i + 1, lenCode, False)
            ...
            for code in codes:
                a = bin(code[0])[2:]
                output += fromDecToBin(len(a), n, False)
                output += a
                b = diction[code[1]]
                output += fromDecToBin(len(b), n, False)
                output += b
            try:
                fe = open(encodepath, "w")
                fe.write(output)
            except IOError:
                print("File "+encodepath + "not accessible")
    except IOError:
        print("File "+filepath+" not accessible")


def lz78_decode(filepath, decodepath):
    try:
        f = open(filepath, "r", encoding="utf-8")
        text = f.read()
        f.close()
        if sorted(list(set(text))) == ['0', '1']:
            ...
            diction = {}
            for i in range(len(alph)):
                let = fromDecToBin(i + 1, lenCode, False)
                diction[let] = alph[i]
                if "0" not in let:
                    lenCode += 1
            codes = []
