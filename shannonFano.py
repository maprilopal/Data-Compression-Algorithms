...
def shannon_fano_in(u, beg, end, p):
    for i in range(beg, end):
        ...
        if sum1 <= sum2:
            ...
            for j in range(i, end):
                u[j] += "1"
            if i - beg > 2:
                shannon_fano_in(u, beg, i, p)
            elif i - beg == 2:
                u[beg] += "0"
                u[i - 1] += "1"
            if end - i > 2:
                shannon_fano_in(u, i, end, p)
            elif end - i == 2:
                u[i] += "0"
                u[end - 1] += "1"
            break
...
def shannon_fano(filepath, encodepath, codepath):
    try:
        ...
        u = shannon_fano_in(u, 0, len(p), list(p.values()))
